...
class TrainingScene: SKScene, SKPhysicsContactDelegate {
    ...
    // Game time trackers
    // ------------------
    fileprivate var lastUpdateTime: CFTimeInterval = 0  // Time when update() was last called
    fileprivate var timeSinceLastAlienSpawned: CFTimeInterval  = 0  // Seconds since the last alien was spawned
    
    // Ship sprite
    // -----------
    // For simplicity's sake, we'll use the spaceship that's provided in Images.xcassets
    // when you start a new Game project
    fileprivate let ship = SKSpriteNode(imageNamed: "rifleTop")
    
    // Physics body category bitmasks
    // ------------------------------
    // We'll use these to determine missle-alien collisions
    fileprivate let missileCategory: UInt32 = 0x1 << 0   // 00000000000000000000000000000001 in binary
    fileprivate let alienCategory: UInt32   = 0x1 << 1   // 00000000000000000000000000000010 in binary
    ...
    override func didMove(to view: SKView) {
        ...
        var error: NSError?
        let backgroundMusicURL = Bundle.main.url(forResource: "background-music", withExtension: "aiff")
        do {
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        let touch = touches.first //anyObject() as! UITouch
    ...
    func spawnAlien() {
        ...
        if Int(arc4random_uniform(2)) == 0 {
            ...
            alienSpriteImage = "Camo Cream-48"
        }
        else {
        ...
        let moveTime = TimeInterval((Int(arc4random_uniform(UInt32(moveTimeRange))) + minMoveTime))
        
        // Send the alien on its way
        let moveAction = SKAction.moveTo(x: alienEndX, duration: moveTime)
        let cleanUpAction = SKAction.removeFromParent()
        alien.run(SKAction.sequence([moveAction, cleanUpAction]))
    }
    
    func fireMissile(_ targetingVector: CGPoint) {
        ...
        let actionMove = SKAction.move(to: missileEndPos, duration: TimeInterval(missileMoveTime))
        let actionMoveDone = SKAction.removeFromParent()
        missile.run(SKAction.sequence([actionMove, actionMoveDone]))
    }
    
    func destroyAlien(_ missile: SKSpriteNode, alien: SKSpriteNode) {
